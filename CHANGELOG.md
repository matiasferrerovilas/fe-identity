...
### Fixed
- CI build (`pnpm run build`) failed on a fresh checkout: `tsc -b`, which runs before `vite
  build`, couldn't resolve `@/routeTree.gen` because that file is generated by
  `@tanstack/router-plugin`'s Vite plugin — which only runs once Vite itself starts, i.e. *after*
  `tsc -b` had already failed. Worked locally by accident because the file already existed on
  disk from a previous `vite build`/`vite dev`. Added `@tanstack/router-cli` and a `routes:generate`
  script (`tsr generate`) wired as `prebuild`, plus an explicit "Generate routes" CI step before
  lint/build so it doesn't only depend on the `prebuild` lifecycle hook firing.
